...
const BlogCard = ({ blog }) => {
  return (
    <Link to={`/blogs/${blog.id}`} className="block bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300">
      <img src={blog.image?.url || blog.image || 'https://via.placeholder.com/400x250'} alt={blog.title} className="w-full h-48 object-cover" />
      <div className="p-6">
        ...
        <div className="flex items-center gap-3 mt-4">
          <img src={blog.author?.avatar || 'https://i.pravatar.cc/150?img=12'} alt={blog.author?.name || 'Author'} className="w-10 h-10 rounded-full" />
          <div>
